% concatenate 2AFC data on detail visibility

% generate path to the data
addpath('./exp5_data/');
files = dir(fullfile('./exp5_data/','/*.mat'));
files = {files.name};
...
for f = 1:size(files,2)
    ...
    % separate out the conditions (col 1 in stim)
...
% save for analysis
save('2afc_result_detail.mat','subjresult');
